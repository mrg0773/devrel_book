...
-- Process Block elements to handle table cells
function Block(block)
    -- If this is a Plain or Para block, process its inlines
    if block.t == "Plain" or block.t == "Para" then
        block.content = Inlines(block.content)
        return block
    end
    return nil
end
...
function Cell(cell)
    if cell.contents then
        for i, block in ipairs(cell.contents) do
            local new_block = Block(block)
            if new_block then
                cell.contents[i] = new_block
            end
        end
    end
    return cell
